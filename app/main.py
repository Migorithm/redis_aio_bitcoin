...
class Keys:
    ...
    def __init__(self, prefix: str = "KingOfTheCode"):
        self.prefix = prefix
...
#A '/refresh' endpoint exists in the app to allow a client 
# to trigger a refresh of the 30-second averages. 
@app.post('/refresh')
...
#We'll use the TS.CREATE RedisTimeSeries command 
# to create the timeseries within our initialize_redis() function:
...
#We're going to store price and sentiment averages in a timeseries with RedisTimeSeries, 
# so we want to make sure that when the app starts up, the timeseries exists.
